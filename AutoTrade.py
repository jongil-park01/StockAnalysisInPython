...
# 크레온 플러스 공통 OBJECT
cpCodeMgr = win32com.client.Dispatch('CpUtil.CpStockCode')  # 주식 정보 조회
...
if __name__ == '__main__':
    try:
        ...
        ##########################################################################
        buy_percent = 0.19
        dbgout("대상종목 매수비율 : " + str(buy_percent))

        ##########################################################################
        # 매수 종목 수 수정 필요
        ##########################################################################
        target_buy_count = 5  # 매수할 종목 수
        dbgout("매수할 종목수 : " + str(target_buy_count))
        dbgout("================")
